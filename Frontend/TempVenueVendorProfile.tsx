...
const VenueVendorProfile = () => {
  ...
  const venueAndVendorData = {
    ...
    reviews: [
      { id: 1, user: "Ananya Sharma", rating: 5, comment: "The venue was breathtaking! The service was exceptional and our wedding was a dream come true." },
      { id: 2, user: "Rahul Desai", rating: 4, comment: "Great location and staff. The food was excellent, though we had a small issue with the sound system." },
      { id: 3, user: "Priya Singh", rating: 5, comment: "Flawless execution of our corporate event. Highly professional and very well organized." },
    ],
    offers: [
      { id: 1, title: "Wedding Package Special", description: "Book our premium wedding package and get a complimentary pre-wedding photoshoot.", expires: "2025-12-31" },
      { id: 2, title: "Mid-week Discount", description: "Receive a 15% discount on all bookings for events held Monday through Thursday.", expires: "2025-11-30" },
      { id: 3, title: "First-time Booker", description: "Get 10% off your total bill on your first booking with us!", expires: "2025-10-15" },
      { id: 4, title: "Corporate Event Offer", description: "15% off on all corporate event bookings of more than 50 guests.", expires: "2025-12-01" },
      { id: 5, title: "Early Bird Offer", description: "Book 3 months in advance and get a complimentary DJ service.", expires: "2026-01-31" },
      { id: 6, title: "Holiday Season Special", description: "Enjoy a 20% discount on bookings for any event in December.", expires: "2025-12-31" }
    ]
  };
  ...
  const handleNextReview = () => {
    setCurrentReviewIndex((prevIndex) => (prevIndex + 1) % venueAndVendorData.reviews.length);
  };

  const handlePreviousReview = () => {
    setCurrentReviewIndex((prevIndex) => (prevIndex - 1 + venueAndVendorData.reviews.length) % venueAndVendorData.reviews.length);
  };

  const handleNextOffer = () => {
    setCurrentOfferIndex((prevIndex) => (prevIndex + 1) % offersInGroupsOfThree.length);
  };

  const handlePreviousOffer = () => {
    setCurrentOfferIndex((prevIndex) => (prevIndex - 1 + offersInGroupsOfThree.length) % offersInGroupsOfThree.length);
  };
          ...
          <div className="card p-6 md:col-span-1">
            <div className="flex justify-around mb-4 border-b border-gray-600">
              <button
                ...
                className={`text-lg font-semibold py-2 px-4 border-b-2 transition-colors duration-300 ${
                  currentView === 'reviews' ? 'border-[#7f34c5] text-[#7f34c5]' : 'border-transparent text-gray-400'
                }`}
              >
                Vendor & Reviews
              </button>
              <button
                ...
                className={`text-lg font-semibold py-2 px-4 border-b-2 transition-colors duration-300 ${
                  currentView === 'offers' ? 'border-[#7f34c5] text-[#7f34c5]' : 'border-transparent text-gray-400'
                }`}
              >
                Offers
              </button>
            ...
            {currentView === 'reviews' ? (
              <>
                ...
                <div className="mt-6 w-full text-left">
                  ...
                  <div className="relative overflow-hidden h-40">
                    <div className="absolute inset-0 flex transition-transform duration-300" style={{ transform: `translateX(-${currentReviewIndex * 100}%)` }}>
                      {venueAndVendorData.reviews.map((review, index) => (
                        <div key={review.id} className="w-full flex-shrink-0 p-4 rounded-lg bg-gray-800">
                          <div className="flex items-center mb-2">
                            <span className="text-yellow-400 text-lg mr-2">
                              {'★'.repeat(review.rating)}
                              {'☆'.repeat(5 - review.rating)}
                            </span>
                            <span className="font-semibold text-gray-50">{review.user}</span>
                          </div>
                  ...
                  <div className="flex justify-center mt-4">
                    <button onClick={handlePreviousReview} className="p-2 rounded-full bg-gray-700 text-gray-300 hover:bg-gray-600 mr-2">
                      &lt;
                    </button>
                    {venueAndVendorData.reviews.map((_, index) => (
                      <button
                        ...
                        className={`w-2 h-2 mx-1 rounded-full ${
                          index === currentReviewIndex ? 'bg-[#7f34c5]' : 'bg-gray-600'
                        }`}
                      ></button>
                    ))}
                    <button onClick={handleNextReview} className="p-2 rounded-full bg-gray-700 text-gray-300 hover:bg-gray-600 ml-2">
                      &gt;
                    </button>
                  </div>
                </div>
              ...
              <div className="mt-4">
                ...
                <div className="relative overflow-hidden h-96">
                  ...
                  >
                    {offersInGroupsOfThree.map((group, groupIndex) => (
                      <div key={groupIndex} className="w-full flex-shrink-0 grid grid-cols-1 gap-4">
                        {group.map((offer) => (
                          <div key={offer.id} className="bg-gray-800 p-4 rounded-lg">
                            ...
                            <p className="text-xs text-gray-500 mt-2">Expires: {offer.expires}</p>
                          </div>
                        ))}
                      </div>
                    ))}
                ...
                <div className="flex justify-center mt-4">
                    <button onClick={handlePreviousOffer} className="p-2 rounded-full bg-gray-700 text-gray-300 hover:bg-gray-600 mr-2">
                      &lt;
                    </button>
                    {offersInGroupsOfThree.map((_, index) => (
                      <button
                        ...
                        className={`w-2 h-2 mx-1 rounded-full ${
                          index === currentOfferIndex ? 'bg-[#7f34c5]' : 'bg-gray-600'
                        }`}
                      ></button>
                    ))}
                    <button onClick={handleNextOffer} className="p-2 rounded-full bg-gray-700 text-gray-300 hover:bg-gray-600 ml-2">
                      &gt;
                    </button>
                  </div>
              </div>
